...
pub fn run(args: &[String]) -> Result<()> {
    ...
    for path_str in paths {
        ...
        let canonical_abs = abs_path.canonicalize().with_context(|| format!("Failed to resolve path: {}", path_str))?;
        
        // Strip the workdir prefix to get the relative path within the repo
        let rel_path = canonical_abs.strip_prefix(workdir)
            .with_context(|| format!("Path is outside of repository working directory: {}", path_str))?;

        index.add_path(rel_path).with_context(|| format!("Failed to add path to index: {}", path_str))?;
